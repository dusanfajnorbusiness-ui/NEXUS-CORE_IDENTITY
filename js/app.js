...
const AccountPanel = ({ color }) => {
  ...
  return (
    <div className="relative font-mono uppercase">
      {!isLogged ? (
        <div ref={authRef}>
          <button onClick={() => setShowAuth(!showAuth)} className="hud-btn border-thick" style={{ borderColor: color, color: color }}>
            INITIATE_SESSION
          </button>
          {showAuth && (
        ...
        <div ref={editRef}>
          <button onClick={() => setIsEditing(!isEditing)} className="flex items-center gap-3 bg-black/80 border border-white/10 p-1 pr-4 rounded-full hover:border-white/30 transition-all">
            ...
            <div className="flex flex-col items-start leading-none text-left">
              <span className="text-[9px] font-black text-white tracking-widest">{user.name}</span>
          ...
          {isEditing && (
            <div className="absolute top-14 right-0 w-80 bg-black/95 border border-white/20 p-6 z-[100] rounded-xl backdrop-blur-xl shadow-2xl animate-in fade-in zoom-in-95">
               <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2">
                 <h4 className="text-[#39FF14] text-[10px] font-black uppercase italic tracking-widest">Profile_Overhaul</h4>
...
const App = () => {
  ...
  useEffect(() => {
    fetch("https://api.github.com/repos/dusanfajnorbusiness-ui/NEXUS-CORE_IDENTITY/commits")
      .then(res => res.json())
      .then(data => {
        setUpdates(data.slice(0, 10).map(c => ({ 
