...
// ==========================================
// 1. MODUL: AUTH & PROFILE ENGINE (NEXUS-CORE_2Mb Edition)
// ==========================================
const AccountPanel = ({ color }) => {
  ...
  // PROTOKOL: Click-Away pre modálne okná
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (authRef.current && !authRef.current.contains(event.target)) setShowAuth(false);
      if (editRef.current && !editRef.current.contains(event.target)) setIsEditing(false);
    };
  ...
  const handleAuth = (selectedTier) => {
    const defaultKeys = { FREE: "NONE", PRO: "NEXUS-150-PRO", PREMIUM: "NEXUS-999-PREMIUM" };
    const newUser = {
      ...user,
      tier: selectedTier,
      subscription: selectedTier === "FREE" ? "LIMITED" : "ACTIVE_FLOW",
      tier_level: selectedTier === "PREMIUM" ? 2 : selectedTier === "PRO" ? 1 : 0,
  ...
  return (
    <div className="flex items-center">
      {!isLogged ? (
        <div className="relative font-mono uppercase" ref={authRef}>
          <button
            onClick={() => setShowAuth(!showAuth)}
            className="hud-btn border-3 rounded-full"
            style={{ borderColor: color, color: color }}
          >
            INITIATE_SESSION
          </button>
          {showAuth && (
            <div className="absolute top-14 right-0 w-80 bg-black border border-white/20 p-6 z-[100] shadow-2xl rounded-xl backdrop-blur-xl">
              <h4 className="text-[#39FF14] text-[10px] mb-4 font-black italic border-b border-white/10 pb-2 uppercase">New_Operator_Auth</h4>
              <div className="space-y-3">
                <button onClick={() => handleAuth("FREE")} className="w-full p-2 bg-white/5 border border-white/10 text-[8px] hover:bg-white/10 flex justify-between"><span>FREE_TIER</span><span>0€</span></button>
                <button onClick={() => handleAuth("PRO")} className="w-full p-2 bg-blue-500/10 border border-blue-500/30 text-[8px] hover:bg-blue-500/20 text-blue-400 flex justify-between"><span>PRO_TIER</span><span>🔑 REQUIRED</span></button>
                <button onClick={() => handleAuth("PREMIUM")} className="w-full p-2 bg-yellow-500/10 border border-yellow-500/30 text-[8px] hover:bg-yellow-500/20 text-yellow-500 flex justify-between"><span>PREMIUM_TIER</span><span>💎 VAULT</span></button>
                <div className="grid grid-cols-2 gap-2 mt-4 opacity-50">
                   <button className="text-[6px] border border-white/10 p-1">Google_Link</button>
                   <button className="text-[6px] border border-white/10 p-1">Email_Auth</button>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="relative font-mono uppercase" ref={editRef}>
          <button onClick={() => setIsEditing(!isEditing)} className="flex items-center gap-3 bg-black/80 border border-white/10 p-1 pr-4 rounded-full hover:border-white/30 transition-all">
            ...
            <div className="flex flex-col items-start text-left leading-none">
              ...
              <span className="text-[6px] opacity-50" style={{ color: color }}>{user.tier} ACCESS</span>
            </div>
          </button>
          {isEditing && (
            <div className="absolute top-14 right-0 w-80 bg-black/95 border border-white/20 p-6 z-[100] shadow-2xl rounded-xl backdrop-blur-xl">
               <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2">
                 <h4 className="text-[#39FF14] text-[10px] font-black italic uppercase">Profile_Overhaul</h4>
               ...
               <div className="space-y-4">
                  <div className="flex flex-col gap-1 text-left">
                    <label className="text-[6px] opacity-40 uppercase text-white">Identity_Mark</label>
                  ...
                  <div className="flex flex-col gap-1 text-left">
                    <label className="text-[6px] opacity-40 uppercase text-white">Tactical_Bio</label>
...
const App = () => {
    ...
    fetch("https://api.github.com/repos/dusanfajnorbusiness-ui/NEXUS-CORE_IDENTITY/commits")
      ...
      .then((data) => {
        const formatted = data.slice(0, 10).map((commit) => ({
          ...
          desc: commit.commit.message.split("\n").slice(1).join(" ") || "No description.",
        }));
  ...
  useEffect(() => {
    const handleOutsideClick = (e) => { if (logRef.current && !logRef.current.contains(e.target)) setIsMenuOpen(false); };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#050505] transition-all duration-700" style={{ borderLeft: `6px solid ${current.color}` }}>
      ...
      {/* 🟢 HUD HEADER (SYMETRIA FIX) */}
      <div className="fixed top-0 left-0 w-full p-4 md:p-6 flex justify-between items-center z-50 bg-gradient-to-b from-black via-black/80 to-transparent">
        <div className="flex items-center gap-4">
          <h1 className="font-mono text-[#39FF14] text-lg font-black tracking-widest">NEXUS-CORE_2Mb</h1>
          <div className="hidden md:block h-4 w-[1px] bg-white/10"></div>
          <span className="hidden md:block text-[7px] font-mono opacity-30 uppercase tracking-widest">Operator: Architect // Node_2Mb</span>
        </div>

        <div className="flex items-center gap-4">
          <button className="hud-btn border-1 rounded-full" style={{ color: statusColor, borderColor: statusColor }}>STATUS_OK</button>
          
          <div className="relative font-mono" ref={logRef}>
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="hud-btn border-2 rounded-full" style={{ color: current.color, borderColor: current.color }}>
              LAST_UPGRADE: {updates[0]?.date || "SYNC"}
            </button>
            {isMenuOpen && (
              <div className="absolute top-14 right-0 w-80 bg-black/95 border border-white/20 shadow-2xl p-5 rounded-xl backdrop-blur-xl animate-in fade-in zoom-in-95">
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar text-left">
                  {updates.map((upd) => (
                    <div key={upd.id} className="border-b border-white/5 pb-2">
                      <div className="flex justify-between text-[6px] opacity-40 italic"><span>#NODE_{upd.id}</span><span>{upd.date}</span></div>
