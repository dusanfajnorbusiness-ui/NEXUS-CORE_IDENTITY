...
window.nexusData = {
  ...
  skills: [
    ...
    { id: 18, name: "Finančná gramotnosť", issuer: "Skillmea", category: "Finance", path: "assets/certs/certifikatySkillmea/skillmea-certifikat-financna-gramotnost-v-kocke.pdf" },
    { id: 20, name: "WebRebel 1 (HTML/CSS/JS)", issuer: "Skillmea", category: "Dev", path: "assets/certs/certifikatySkillmea/skillmea-certifikat-webrebel-1-html-css-javascript.pdf" }
  ],

  dimensions: {
    "01": { name: "identity", color: "#39FF14", quote: "Keď som, tak som, viem, že som.", content: "Jadro, autenticita, 'Keď som, tak som.'", proContent: window.id01Data?.proContent || "OFFLINE", premiumContent: window.id01Data?.premiumContent || "OFFLINE" },
    "02": { name: "workFlow-leader", color: "#FFD700", quote: "Efektivita v prítomnosti.", content: "Operatíva, výkon, denné úlohy.", proContent: window.id02Data?.proContent || "OFFLINE", premiumContent: window.id02Data?.premiumContent || "OFFLINE" },
    "03": { name: "cash-engine", color: "#FFAA00", quote: "Peniaze sú palivo slobody.", content: "Správa toku peňazí a dlhov.", proContent: window.id03Data?.proContent || "OFFLINE", premiumContent: window.id03Data?.premiumContent || "OFFLINE" },
    "04": { name: "revenue-architect", color: "#E0A800", quote: "Projektovanie zisku.", content: "Budovanie systémov a predaj Access Key.", proContent: window.id04Data?.proContent || "OFFLINE", premiumContent: window.id04Data?.premiumContent || "OFFLINE" },
    "05": { name: "asset-matrix", color: "#D4AF37", quote: "Tichý rast hodnôt.", content: "Dlhodobá stratégia a 'Kiyosaki mindset'.", proContent: window.id05Data?.proContent || "OFFLINE", premiumContent: window.id05Data?.premiumContent || "OFFLINE" },
    "06": { name: "data-absorber", color: "#BF00FF", quote: "Nasávanie vedomostí ako procesor.", content: "Spracovanie 38 certifikátov a vedomostí.", proContent: window.id06Data?.proContent || "OFFLINE", premiumContent: window.id06Data?.premiumContent || "OFFLINE" },
    "07": { name: "skill-tree", color: "#8A2BE2", quote: "Vetvenie schopností.", content: "Kariérny status a Skill Tree (HTML/CSS).", proContent: window.id07Data?.proContent || "OFFLINE", premiumContent: window.id07Data?.premiumContent || "OFFLINE" },
    "08": { name: "empty-book", color: "#00FFFF", quote: "Čistá strana pre tvorbu.", content: "Absorpcia informácií, 'Prijímací režim'.", proContent: window.id08Data?.proContent || "OFFLINE", premiumContent: window.id08Data?.premiumContent || "OFFLINE" },
    "09": { name: "sergeant-core", color: "#FF003C", quote: "Disciplína v akcii.", content: "CoD Zombies, YouTube, Akcia.", proContent: window.id09Data?.proContent || "OFFLINE", premiumContent: window.id09Data?.premiumContent || "OFFLINE" },
    "10": { name: "non-identity", color: "#0044FF", quote: "Boh je Ticho.", content: "Ticho, Boh, Reset, Rodina, Základňa.", proContent: window.id10Data?.proContent || "OFFLINE", premiumContent: window.id10Data?.premiumContent || "OFFLINE" },
    "11": { name: "the-recovery", color: "#FFBF00", quote: "Cesta von z Matrixu.", content: "Manifest Architekta. Ako von zo schizofrénie a insomnie.", proContent: window.id11Data?.proContent || "OFFLINE", premiumContent: window.id11Data?.premiumContent || "OFFLINE" }
  }
};